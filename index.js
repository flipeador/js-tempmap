/*
    ...
    https://github.com/flipeador/js-tempmap
*/
...
export class TempMap extends Map
{
    ...
    /**
     * Get an element from the map if it exists, otherwise sets and returns `defval`.
     * @param key The key associated with the element.
     * @param defval The default value or a `function(key,map)` that generates it.
     * @param {undefined|Number} timeout The timeout, in milliseconds. See {@link TempMap.set}.
     * @param {Boolean} refreshTimeout Whether to refresh the element's timeout, if it exists.
     * @return The element associated with the specified key, or `defval` if the element does not exist.
     */
    ensure(key, defval, timeout, refreshTimeout=true)
    {
        const item = this.#getItem(key, refreshTimeout);
        if (item) return item.value;
        return this.#setItem(key, null, getValue(defval, key, this), timeout);
    }
    ...
    *#iter()
    ...
    {
        return refreshTimeout
            ? this.#refreshItemTimeout(key, super.get(key))
            : super.get(key);
    }
    ...
    #refreshItemTimeout(key, item)
    {
        if (item?.timer)
        {
            ...
            // Browser
            else item.timer.timer = setTimeout(() => {
                super.delete(key);
            }, item.timer.timeout);
        }
        return item;
    ...
    /**
     * Set or clear the timeout of an item and returns it.
     */
